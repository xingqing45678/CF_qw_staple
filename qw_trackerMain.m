function [results] = qw_trackerMain(p, im, bg_area, area_resize_factor)
        ...
        if p.visualization %为0时不画图，1时画图
            if 0%~isToolboxAvailable('Computer Vision System Toolbox')
                ...
                if scale_factor > 2
                    ...
                    shizi = scale_factor*5;
                end
                    if frame == 1,  %first frame, create GUI
                        figure
                        im_handle = imshow(im);%imagesc(uint8(im));
                        rectt_handle = rectangle('Position',rect_position, 'EdgeColor','r');
%                         rectb_handle = rectangle('Position',rect_position_padded, 'LineWidth',2, 'LineStyle','--', 'EdgeColor','b');
                        tex_handle = text(5, 18, strcat('#',num2str(frame)), 'Color','y', 'FontSize',10);
                        hold on;
                        pos_handle = plot(pos(2),pos(1),'r+','MarkerSize',shizi);
                        drawnow;
                    else
